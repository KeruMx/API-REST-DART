...
class UsersTypeController extends ResourceController{
  ...
  Future<Response> getUsersType() async{
    final usersTypeQuery = Query<UsersType>(context);
  ...
  Future<Response> getActivityById(@Bind.path('idUsersType') int idUTy) async{
    final usersTypeQuery = Query<UsersType>(context)..where((a)=>a.idUsersType).equalTo(idUTy);
    final resUsersType = await usersTypeQuery.fetch();
  ...
  Future<Response> insUsersType() async{
    final objUsersType = UsersType()..read(await request.body.decode());
    final query = Query<UsersType>(context)..values = objUsersType;
  ...
  Future<Response> upUsersType(@Bind.path('idUsersType')int idUTy) async{
    final objUsersType = UsersType()..read(await request.body.decode());
    final queryUsersType = Query<UsersType>(context)..where((a)=> a.idUsersType).equalTo(idUTy)..values = objUsersType;
  ...
  Future<Response> delUsersType(@Bind.path('idUsersType')int idUTy) async{
    final queryUsersType = Query<UsersType>(context)..where((a)=> a.idUsersType).equalTo(idUTy);
